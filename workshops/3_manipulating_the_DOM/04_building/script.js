 ...
 *
 * TASK: 
 * - Set the first part of the EULA (the variable `eula`) as the innerHTML of `eulaText`.
...
/* These two events prevent the user from selecting the EULA text by
   ...
   cursor to select the text). You don't have to change anything here */
...
eulaButtonConfirm.addEventListener("pointerdown", (event) => {
  const userInput = eulaTextArea.value; // Remember, we get the value from an <input> or <textarea> with `.value`
...
eulaTextArea.addEventListener("input", (event) => {
  // If you want to do something when the user inputs text.
});
